...
- **D-301**: 11 design tradeoffs ratified (Stage 3 IDs labelled D-001 .. D-011 in spec.md tradeoff table covering CLI framework, probe corpus format, SARIF emission, LlmProvider abstraction, test framework, probe execution model, mask hook, MCP schema sourcing, probe corpus versioning, config loader, logger). → spec.md § Tradeoffs, ADR-0003 §5
...
### Stage 4 Tasks v2 accepted
- **D-401**: 39 tasks across 9 layers (L0 Foundation → L9 Phase α gate), build order strict topological (L0 → L1 → L2/L3 parallelable → L4/L5/L6 parallelable → L7 → L8 → L9). → tasks.md § WBS overview
- **D-402**: All 31 EARS AC + 11 tradeoffs D-001~D-011 + 8 Phase α AC (incl. AC-α-8 history sweep as out-of-scope marker) literal mapped to tasks; orphan AC zero. → tasks.md § AC coverage matrix
- **D-403**: 6-point objective-evaluation patch round applied prior to kickoff approval — (1) lint tool drop, ci pipeline = `pnpm tsc --noEmit` + `pnpm vitest run --coverage` + `pnpm audit --audit-level=high`; (2) commander built-in `showSuggestionAfterError()` replaces self-rolled Levenshtein in T-31; (3) `yaml` (eemeli/yaml, ISC license) adopted in T-01 batch to honor D-002 YAML-format literal lock; (4) ADR count to 5 via T-09 ADR-0005 (MCP spec upstream pin) + T-38 ADR-0004 (Golf Scanner audit outcome); (5) AC-α-2 5-consecutive-green-CI as stream condition wording in T-33/T-39; (6) vitest coverage thresholds (lines/functions/branches/statements ≥ 80) wired in T-02 vitest.config.ts. → tasks.md F-1 through F-6 patch records
- **D-404**: Deps list locked = `commander`, `zod`, `vitest`, `@vitest/coverage-v8`, `typescript`, `@types/node`, `tsx`, `yaml`; single batch in T-01 per spec.md § cost-constraint and internal package-adoption gate. No further dep additions during Phase 1 except via explicit ADR-gated review.
